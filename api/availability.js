...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Создаем карту соответствия ID товара из каталога -> номер домика
    ...
    // Проверяем каждую активную сделку и запрашиваем её товары
    for (const deal of deals) {
      ...
      // Получаем товарные позиции для текущей сделки
      const productRows = await getDealProductRows(webhook, deal.ID);
      for (const row of productRows) {
        const prodId = Number(row.PRODUCT_ID);
        // Проверяем по ID товара из каталога или извлекаем номер из названия товара
